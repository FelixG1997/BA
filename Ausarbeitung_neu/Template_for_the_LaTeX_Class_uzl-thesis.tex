...
Indirekter Beweis: \\
Nehmen an, der Verfolger bewegt sich mit $v_{verfolger} < v_{max}$. Dies ist äquivalent dazu, dass der Verfolger an seiner aktuellen Position eine Zeit $\tau$ wartet und sich dann mit $v_{max}$ weiterbewegt, um dann das nächste Ziel $s$ einzuholen. Dabei befindet sich $s$ auf der anderen Achse. Nach der Wartezeit erreicht der Verfolger an Zeitpunkt $t_1$ den Ursprung und holt das Ziel $s$ an der Position $p$ zum Zeitpunkt $t_2$ ein. \\
...
Mit diesen Bedingungen wird im Beipiel von Abbildung \ref{fig:BF1} $2$ Berechnungen eingespart (siehe Abbildung \ref{fig:BF2}). Bei jeder Permutation wird am Anfang und am Ende der Ursprung eingerechnet. \\
...
\textcolor{red}{TODO: Überarbeiten, diese ist nun viel besser}
...
\item
Brute-Force-Algorithmus auf Effektivität testen mit zufällig generierten Inputs für $n=10,12,13,14$
\item
...
\begin{align*}
n&=5,25,50,75,100\\
(v_{verfolger},v_i)&=(200,20),(100,40),(59,60)\\
